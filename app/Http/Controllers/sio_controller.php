...
class sio_controller extends Controller
{
    ...
    public function updated_receipts_all(){
        try {
            DB::connection('SIO')->update('exec updated_cia');
            return response()->json([
                'status' => true,
    ...
    public function detail_receipts(Request $request)
    {
        ...
        if (!$request) {
            ...
            $details_receipts = DB::connection('SIO')->table('tbl_payment_receipts')
    ...
    public function update_receipts_cia(Request $request){
        ...
        try {
            DB::connection('SIO')->update('exec updated_cia_receipts ?,?', [
    ...
    public function detail_partners(Request $request)
    {
        ...
        if (!$request) {
            ...
            $details_partener = DB::connection('SIO')->table('tbl_partners')
    ...
    public function updated_partners(Request $request)
    {
        ...
        try {
            DB::connection('SIO')->update('exec update_partners ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?', [
                $data->id_partener,
                strtoupper($data->name),
                strtoupper($data->last_name),
                strtoupper($data->mother_last_name),
                $data->id_sex,
                $data->birthdate,
    ...
    public function created_partners(Request $request)
    {
        ...
        $details_parteners = DB::connection('SIO')->table('tbl_partners')
        ...
        if (sizeof($details_parteners) == 0) {
            try {
                sio_partners::insert([
                    ...
                    'mother_last_name' => strtoupper($request->mother_last_name),
    ...
    public function updated_status_partners(Request $request)
    {
        $rules = [
            'id_partener' => 'required',
        ];
        ...
        try {
            ...
            switch ($id_status->id_status) {
                case 15:
                    sio_partners::where('id_partener', $request->id_partener)->update([
                        'id_status' => 4
                    ]);
                    break;
                case 4:
    ...
    public function detail_origin_account(Request $request)
    {
        ...
        if (!$request) {
            ...
            $detail_origin_accounts = DB::connection('SIO')->table('origin_account_view')->where('id_origin_accounts', $request->id_origin_accounts)->first();
    ...
    public function updated_origin_account(Request $request){
        ...
        try {
            DB::connection('SIO')->update('exec updated_origin_accounts ?,?,?,?,?,?,?', [
    ...
    public function updated_cia_details(Request $request){
        ...
        try {
            DB::connection('SIO')->update('exec updated_cia_details ?,?,?,?', [
                $request->name_cia,
                $request->abbreviation_cia,
                $request->rfc_cia,
                $request->id_cia
    ...
    public function detail_cia(Request $request)
    {
        ...
        if (!$request) {
            ...
            $details_cia = DB::connection('SIO')->table('cia_view')->where('id_cia', $request->id_cia)->first();
    ...
    public function ct_states_accounts()
    {
     ...
     public function ctl_municipality(Request $request)
     {
         try {
             set_time_limit(0);
             $ctl_municipality = DB::connection('SIO')->table('ctl_municipality')->where('id_states', $request->id_states)->orderBy('id_municipality', 'desc')->get();
             return response()->json([
                 'status' => true,
